      ...
      <div class="card border-0">
          ...
          <div class="card-footer">
              <div class="">
                  ...
                  <button id="puntosMas" class="btn btn-outline-secondary btn-sm " href="${pathpts}/mas"><i class="fa-solid fa-caret-up"></i></button>
                  <small id="points" class="text-center mb-1">${markerPins[idx].pmas - markerPins[idx].pmenos} </small>
                  <button id="puntosMenos" class=" btn btn-outline-secondary btn-sm " href="${pathpts}/menos"><i class="fa-solid fa-caret-down"></i></button>
              </div>
          </div>
      </div>
  </div>` 
...
for (var i = 0; i < markerPins.length; i++) {
  marker = new L.marker([markerPins[i].lat, markerPins[i].lng])
    .addTo(map)
    .bindPopup(RenderPopUp(markerPins, i));
    // SetPointClick();
}

// function SetPointClick(){
  $("map").ready(function(){
